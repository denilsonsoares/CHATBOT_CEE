...
import pandas as pd
import os
from utils import configure_genai, configure_openai, count_tokens_in_folder, process_files_and_save
...
opcao = st.sidebar.selectbox("Escolha a aba:", ["Áreas de Atuação", "Setores", "Remuneração", "Requisitos", "Extração de Dados"])
...
# Aba de Extração de Dados
    ...
    modelo_selecionado = st.selectbox("Selecione o modelo:", ["gpt-3.5-turbo", "gemini-1.5-flash"])
    ...
    if st.button("Extrair Dados"):
        ...
        total_tokens_usados, nome_arquivo_gerado = process_files_and_save(os.path.join('vagas', pasta_selecionada), openai.api_key, modelo_selecionado)
